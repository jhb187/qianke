...
public class WxConfig {
    ...
    public String getJsapiTicketUrl() {
        if (cacheService.getCache("access_token") != null) {
            return "/cgi-bin/ticket/getticket?access_token=" + cacheService.getCache("access_token").toString()
                    + "&type=jsapi";
        } else {
            log.warn("从cache中获取access_token失败！！！！");
            Object token = cacheService.getCache("access_token");
            return "/cgi-bin/ticket/getticket?access_token=" + token.toString()
                    + "&type=jsapi";
        }
    }
